...
export default function Header({ isLive = true }: { isLive?: boolean }) {
  ...
  return (
    <header className="w-full bg-finanzar-surface border-b border-finanzar-border shadow-sm sticky top-0 z-30">
      {/* Fila superior: marca + indicador de mercados */}
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 h-16 flex items-center justify-between">
        <Link
          to="/"
          className="group flex items-baseline focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-finanzar-accent rounded-xs"
        >
      ...
      <div className="w-full border-t border-finanzar-borderSubtle bg-finanzar-bg">
        <nav className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-2 flex justify-center">
                ...
                <li key={link.to}>
                  <Link
                    ...
                    className={`inline-block px-3 py-1 rounded-sm text-xs transition-colors ${
                      isActive
                        ? "text-finanzar-primary font-semibold bg-finanzar-surface border border-finanzar-border"
